# laboratory_checker.py
# Streamlit web UI for Laboratory Capacity Checker (IFC + Shapely)
# - IFC upload required (no auto)
# - Sidebar: Student number input
# - Caption line above the report
# - CSV download only
# - Report (GOOD/BAD lists) BEFORE the table
...
def is_laboratory(room: RoomRec) -> bool:
    ln, nm = _lower(room.longname), _lower(room.name)
    return (ln == "laboratory") or (nm == "laboratory")
...
def render_results(title, rooms_filtered, max_cap, out_csv_name, code_text):
    ...
    st.title("👨‍🔬Laboratory Capacity Checker")
    ...
    # Detailed lists BEFORE the table
...
# -------------------------
# Public entry point (for tabs)
# -------------------------
def run_laboratory_checker(ifc=None):
    if ifc is None:
    ...
    # Only keep parameters here (no file_uploader)
    with st.sidebar:
        st.header("Parameters")
        max_cap = st.number_input(
            "Student number (max capacity per room)",
            min_value=1, max_value=10000, value=DEFAULT_MAX_CAPACITY, step=1,
            help="Urban labs/workshops: 24 (rural: 18)."
        )

    # Use the shared IFC object directly
    rooms = collect_rooms(ifc)
    chairs = collect_chairs(ifc)

    lab_rooms = {k: r for k, r in rooms.items() if is_laboratory(r)}
    if not lab_rooms:
        lab_rooms = {k: r for k, r in rooms.items()
                     if _lower(r.longname) == "room" or _lower(r.name) == "room"}

    lab_chairs = [c for c in chairs if chair_matches(c, STANDARD_CHAIR_NAMES["laboratory chair"])]
    assign_chairs_to_rooms(lab_rooms, lab_chairs, z_tol_m=DEFAULT_Z_TOL_M)

    render_results("", lab_rooms, max_cap,
                   "laboratory_capacity_report.csv", "2-1-2")
...
if __name__ == "__main__":
    ...
    st.set_page_config(page_title="Laboratory Capacity Checker", layout="wide")
    run_laboratory_checker()
